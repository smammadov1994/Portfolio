...
export const chapters=[
 ...
 {name:'Rewrite',title:'Make the next story yours.',copy:'Retrieved examples meet a new topic, place, and audience. A grounded first draft begins to take shape.'},
];
const references=[
 {image:'swipebuilder-food-creator.png',name:'Worth the detour.',tag:'food diary'},
 {image:'swipebuilder-city-creator.png',name:'Take the side street.',tag:'city walks'},
 {image:'swipebuilder-coast-creator.png',name:'Stay for the view.',tag:'quiet places'},
];
const clamp=x=>Math.max(0,Math.min(1,x));
const ease=x=>{x=clamp(x);return x*x*(3-2*x)};
const mix=(a,b,t)=>a+(b-a)*t;
function PaperCard({item,index,width,height}){
 return <div style={{width,height,padding:5,background:'#f8f7f2',border:'1px solid #b8b9b1',boxShadow:'3px 10px 18px #383b2f25',position:'relative',borderRadius:9}}>
  <div style={{position:'absolute',width:54,height:19,left:'calc(50% - 27px)',top:-9,zIndex:2,background:'#d6d4c8bb',border:'1px solid #b4b2a655',transform:'rotate(-5deg)'}}/>
  <div style={{position:'relative',width:'100%',height:'100%',overflow:'hidden',borderRadius:4}}>
   <Img src={`/assets/${item.image}`} alt={`Fictional creator sharing ${item.tag}`} style={{width:'100%',height:'100%',display:'block',objectFit:'cover',objectPosition:'center top',filter:'grayscale(1)'}}/>
   <div style={{position:'absolute',inset:0,background:'linear-gradient(transparent 48%,#141414b3 85%,#141414d9)'}}/>
   <div style={{position:'absolute',left:16,right:16,bottom:22,color:'#fff',fontFamily:"'Manrope',sans-serif",textShadow:'0 1px 4px #0008'}}>
    <div style={{fontSize:15,letterSpacing:1,marginBottom:10}}>0{index+1} / {item.tag}</div>
    <div style={{fontSize:25,fontWeight:600,lineHeight:1.2}}>{item.name}</div>
    <div style={{marginTop:14,display:'flex',alignItems:'center',gap:7,fontSize:13,opacity:.85}}><span style={{display:'inline-block',width:6,height:6,borderRadius:'50%',background:'currentColor'}}/>a moment worth saving</div>
   </div>
  </div>
 </div>;
}
export function SwipeFilm({compact=false}){
 ...
 const cw=compact?230:220,ch=compact?410:390;
 const paper={background:'#f8f7f2',border:'1px solid #bfc0b5',boxShadow:'4px 10px 20px #353c2d18'};
 const query='hidden neighborhood food spots';
 const script='Skip the main street.\nFind the little places\nthat make a city yours.';
 ...
 return <AbsoluteFill style={{background:'#eeedeb',color:'#3f433e',fontFamily:"'Caveat',cursive",lineHeight:1.2,overflow:'hidden'}}>
  ...
  <div style={{position:'absolute',inset:compact?'115px 32px 105px':'105px 60px 105px',border:'1px dashed #aeb3a6',borderRadius:5,background:'#dddcd422',opacity:boardOpacity,transform:`scale(${.95+board*.05})`}}>
   <div style={{position:'absolute',top:18,left:25,fontSize:compact?30:25}}>food & places to remember</div>
  ...
  {references.map((item,i)=>{
   ...
   const initialX=compact?[85,420,265][i]:[135,485,835][i];
   const initialY=compact?[160,180,315][i]:[130,100,130][i];
   const boardX=compact?[95,450,290][i]:[155,485,815][i];
   const boardY=compact?[190,190,425][i]:[170,170,170][i];
  ...
  <div style={{position:'absolute',bottom:compact?75:80,left:30,right:30,textAlign:'center',fontSize:compact?33:30,opacity:1-explore,transform:`translateY(${(1-board)*4}px)`}}>{board>.65?'“What if we took the side street?”':'a few things worth another look'}</div>
  <div style={{position:'absolute',left:compact?52:120,right:compact?52:120,top:compact?125:105,opacity:transcriptOpacity,padding:'15px 25px',border:'1px solid #b6bcb0',borderRadius:30,fontSize:compact?26:27,background:'#f6f5ef'}}><span style={{marginRight:20,color:'#777f72'}}>⌕</span>{query.slice(0,Math.floor(clamp((f-365)/50)*query.length))}<span style={{opacity:f%25<13?1:0}}>│</span><span style={{float:'right',fontSize:compact?21:20,color:'#7a8073',paddingTop:4}}>search by meaning</span></div>
  <div style={{position:'absolute',left:compact?335:510,right:compact?35:120,top:compact?230:200,paddingLeft:compact?20:32,borderLeft:'1px solid #b7bfb0',opacity:transcriptOpacity,transform:`translateY(${(1-explore)*30}px)`}}>
   ...
   <div style={{fontSize:compact?37:39,lineHeight:1.15,minHeight:100,margin:'10px 0 22px',whiteSpace:'pre-line'}}>{'“Take the side street.\nFollow the steam.”'.slice(0,Math.floor(clamp((f-395)/60)*43))}</div>
   <div style={{fontSize:compact?23:20,color:'#7b8373'}}>00:04 / the discovery</div>
   <div style={{fontSize:compact?32:33,lineHeight:1.2,marginTop:10,opacity:ease((f-445)/28)}}>A tiny noodle spot.<br/>A seat by the kitchen.</div>
   <div style={{fontSize:compact?27:25,color:'#69765e',marginTop:28,opacity:ease((f-480)/30)}}>the idea: a local discovery<div style={{height:2,background:'#8d9783',width:`${ease((f-482)/35)*90}%`,marginTop:5}}/></div>
  </div>
  <div style={{position:'absolute',left:compact?38:105,top:compact?130:190,width:compact?w-76:290,opacity:rewrite,transform:`translateX(${(1-rewrite)*-40}px)`}}>
   <div style={{fontSize:compact?25:22,color:'#7b8373'}}>a new place + audience</div>
   <div style={{fontSize:compact?39:45,lineHeight:1.1,marginTop:16}}>A neighborhood food walk.</div>
   <div style={{fontSize:compact?26:28,color:'#6e7668',lineHeight:1.35,marginTop:18}}>For curious visitors looking<br/>beyond the usual stops.</div>
  ...
  <div style={{...paper,position:'absolute',left:compact?65:510,top:compact?340:130,width:compact?620:545,minHeight:compact?385:415,padding:compact?'28px 34px':'32px 40px',opacity:rewrite,transform:`translateY(${(1-rewrite)*85}px) rotate(${(1-rewrite)*-9+1}deg)`}}>
   ...
   <div style={{fontSize:compact?29:28,lineHeight:1.3,opacity:ease((f-671)/30)}}>A corner cafe. A shared table.<br/>Your next favorite place.</div>
   <div style={{fontSize:compact?23:21,color:'#7b8373',marginTop:18,opacity:ease((f-691)/25)}}>a starting point. make it yours.</div>
  </div>
  <div style={{position:'absolute',left:28,right:28,bottom:22,fontSize:compact?22:19,textAlign:'center',color:'#797e74'}}>Fictional creators · food diaries & places worth sharing</div>
 </AbsoluteFill>;
}
function SwipeStory(){
